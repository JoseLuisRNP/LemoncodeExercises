...
const avgCalculator = (numbers: number[]) => {
  return Number((numbers.reduce((total, number) => total + number)/numbers.length).toPrecision(3));
}

const summarizeClassRoom = (studentList: Student[]) => {
  const sumarize: StudentSummary[] = []
  studentList.forEach(student => {
    const studentSummary:StudentSummary = {
      name: student.name,
      higherCalification: Math.max(...student.califications),
      avgCalification: avgCalculator(student.califications),
    }
    sumarize.push(studentSummary); 
  });
  return sumarize;
};
